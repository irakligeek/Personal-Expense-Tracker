import { spendingDetails } from "../(dashboard)/lib/lib";
...
export default function SpendingBreakdown({ category, month, year }) {
  const details = spendingDetails(category);
  return details && details.length > 0 ? (
    <div className="w-full ">
      <div className="grid grid-cols-3 text-sm text-secondary 
      ...
      <ul className="max-h-[1024px] overflow-y-scroll text-secondary border-b border-zinc-600">
        {details.map((detail, index) => (
          <li key={index} className="text-sm grid grid-cols-3 pb-3 last:pb-0 border-b
